...
describe(getLimitsForOwner.name, () => {
    it('returns null when no owner limit exists', async () => {
        ...
        const result = await getLimitsForOwner({ db, ownerId: ownerId123 });
    ...
    it('returns storage limit for the right owner', async () => {
        ...
        const result123 = await getLimitsForOwner({ db, ownerId: ownerId123 });

        assert(result123.ok);
        expect(result123.value).toBe(123);

        const result456 = await getLimitsForOwner({ db, ownerId: ownerId456 });
